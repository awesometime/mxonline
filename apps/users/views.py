...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            user_message.user = user_profile.id  # UserMessage.user是user的int型的id号,而不是一个user对象
...
class LogoutView(View):
    def get(self, request):
        # django自带的logout
        logout(request)
        # 重定向到首页, 这样就可以直接用index作代表的IndexView函数来处理显示对应html
        # 不需要在这儿再写一遍IndexView函数的内容
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                ...
                if user.is_active:
                    ...
                    # 跳转到首页 user request会被带回到首页
                    # 这样就可以直接用index作代表的IndexView函数来处理显示对应html
                    # 不需要在这儿再写一遍IndexView函数的内容
                    return HttpResponseRedirect(reverse("index"))
                # 即用户未激活跳转登录，提示未激活
                else:
...
class ModifyPwdView(View):
    """未登录状态下修改密码的view, 也就是用户忘记了登录密码, 登录不进去, 通过邮箱修改成新密码后再登陆"""
    def post(self, request):
...
class UserInfoView(LoginRequiredMixin, View): # 先验证登录与否
    """个人中心页"""
    login_url = '/login/'
...
class UploadImageView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        # todo 理解一下 ModelForm用法  UploadImageForm是一个ModelForm  视频10.4
        image_form = UploadImageForm(
            request.POST, request.FILES, instance=request.user) # 进阶方法 instance
        if image_form.is_valid():
            image_form.save()  # 进阶方法
            # 普通方法:
...
class UpdatePwdView(LoginRequiredMixin, View):
    """
    在个人中心修改用户密码,也就是登录状态下,用户想修改一个新的密码

    在个人中心修改用户密码执行过程
    第一步:
        在 usercenter-base.html页面点击修改密码,deco-user.js代码的 function search_click()函数
        向 /users/update/pwd发起一个ajax request请求,ajax请求不会返回html页面,只能返回json,浏览器地址栏也不会变化
        前端弹出修改密码栏小窗 及小窗消失 (具体哪部分代码控制不知道 ???
        猜是usercenter-base.html页面语法决定如果向
        /users/update/pwd发起请求的效果就是弹一个窗口出来)
    第二步:
        urls控制器匹配到/users/update/pwd进入处理函数UpdatePwdView中,
        处理结果返回给前端js代码,js再做相应处理,给用户相应提示信息
    """
    login_url = '/login/'
    redirect_field_name = 'next'
...
class SendEmailCodeView(LoginRequiredMixin, View):
    """发送邮箱验证码的view:"""
    def get(self, request):  # todo 为啥是get ???
        # 取出需要发送的邮件
...
# 取消收藏的机构讲师课程是 usercenter-base.html 中的js代码
# jQuery 向organization.views.AddFavView 接口发送一个请求取消收藏
...
class MyFavTeacherView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # 上面的fav_teachers只是存放了id。我们还需要通过id找到机构对象
...
class MyFavCourseView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # 上面的fav_orgs只是存放了id。我们还需要通过id找到课程对象
...
class MyMessageView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # UserMessage中user是一个int型的id,而不是user对象,所以user=request.user.id
