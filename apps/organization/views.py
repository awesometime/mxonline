...
三要素( 页面 | js代码 | 将要发送请求的url ) (base.html | deco-common.js | request_url)
...
class OrgView(View):
    def get(self, request):
        ...
        if search_keywords:
            ...
            # or操作使用Q
            all_orgs = all_orgs.filter(Q(name__icontains=search_keywords) | Q(
        ...
        orgs = p.page(page)
        # q=orgs.object_list  object_list是Paginator库的方法
...
class OrgHomeView(View):
   ...
   机构首页
    ...
    def get(self, request, org_id):   # urls中传入org_id 此处接收该参数
        # 向前端传值，表明现在在home页
        current_page = "home"
        # 根据id取到课程机构
        ...
        # django ORM 用法 场景:涉及外键时
        # course_set  teacher_set
        # Course表名首字母小写下划线加set    作用:将所有课程取出
...
class TeacherDetailView(View):
    ...
    def get(self, request, teacher_id):
        ...
        # 判断如果当前用户已经收藏过页面显示已收藏 否则显示收藏
        # 前提当然是用户处于登录状态user=request.user,应该会完成用户登录判断