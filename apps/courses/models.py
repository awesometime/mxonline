...
class Course(models.Model):
    ...
    #     # 获取课程章节数的方法
    #     # 使用场景: 外键  类名_set 返回queryset对象
    #     return self.lesson_set.all().count()

    # 获取学习这门课程的用户
    # 替代标签:course.usercourse_set.get_queryset|slice:":1"
    # def get_learn_users(self):
    #     # 类名_set,返回queryset对象,谁的里面添加了它做外键，他都可以取出来
    #     # operation 中的Usercourse表
    #     return self.usercourse_set.all()[:5]  # 页面只展示5个

    def __str__(self):
        return self.name  # 后台显示具体Course名字,而不是Course object
